...
#
# Ported into dates on 2026-08-29 from hl7 (`scripts/check-no-emdash.sh` at
# `3d33917`), which is itself the TEXT-ONLY variant first written in knowledgebase:
# no NUL-byte partition, because this repo tracks no binaries. Verified before the
# port over the entire tracked tree, which at `aa3f044` is TWO FILES (`LICENSE` and
# `README.md`): both read as UTF-8, neither holds a NUL byte, and neither carried an
# em dash. So this gate changed no content. It exists purely to stop a regression,
# which is the only reason to add it to a clean repo.
#
# TWO DELIBERATE OMISSIONS FROM THE HL7 COPY, both because this repo is not that repo:
#
#   1. hl7 excludes `vendor/hl7-v2ig/message-structure/` and
#      `vendor/hl7-v2ig/control-manifests/` from the scan, because it commits an HL7
#      publication byte for byte and one of those files carries the banned character
#      inside HL7's own description text. `dates` vendors nothing, so the exclusion
#      list here is THIS SCRIPT AND NOTHING ELSE. Do not copy the vendor prefixes
...
# text file to be classified out of the scan.
#
...
#                                      ARE the commit message that lands on main)
#
